...
const InviteUser = () => {
    ...
    const returnUrl = encodeURIComponent(
        `${BASE_ROUTES.INVITE_URL.replace(":inviteCode", inviteCode)}`
    )
                ...
                <div className="flex flex-col gap-6 ">
                    <Card>
                        <CardHeader className="text-center">
                            ...
                            <CardDescription>Looks like you need to be logged into your TeamSync account to
                                join this Workspace.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            {isPending ? (
                                <Loader className="!w-11 !h-11 animated-spin place-self-center flex" />
                            ) : (
                                <div>
                                    {user ? (
                                        <div className="flex items-center justify-center my-3">
                                            <form onSubmit={handleSubmit}>
                                                <Button type="submit" disabled={isLoading} className="!bg-green-700 !text-white !text-[23px] !h-auto !cursor-pointer ">
                                        ...
                                        <div className="flex flex-col gap-2 items-center md:flex-row">
                                            ...
                                            <Link className="flex-1 w-full text-base" to={`/?returnUrl=${returnUrl}`}>
                                                <Button variant={"secondary"} className="w-full border">Login</Button>
                                            </Link>
                                        </div>
                                    )}
                                </div>
